...
export default {
  name: 'MoviesList',
  props: { // через props дочерняя компонента получает данные от родительской
    list: {
      type: Object,
      default: () => ({})
    }
  ...
  computed: {
    isExist() { // проверка, пустой ли объект (! проверки лучше выносить в computed, если они загромождают разметку)
